...
st.title("Web")
st.write("Click it to RUN")
if st.button("Clicked"):
    st.success("it's running")
    ...
    def show_thanks_popup():
        thanks_popup = tk.Toplevel()
        thanks_popup.title("感谢")
        thanks_popup.protocol("WM_DELETE_WINDOW", on_closing)
        screen_width = thanks_popup.winfo_screenwidth()
        screen_height = thanks_popup.winfo_screenheight()
        ...
        thanks_popup.after(1500, lambda: close_all_other_windows())
    
    
    def create_child_popup(parent):
